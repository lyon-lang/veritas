...
async function loadRecent() {
  try {
    const data = await api.storage.local.get('verifications');
...
async function saveVerification(data) {
  try {
    const stored = await api.storage.local.get('verifications');
    ...
    await api.storage.local.set({ verifications: verifications.slice(0, 50) });
    loadRecent();
  } catch (error) {
